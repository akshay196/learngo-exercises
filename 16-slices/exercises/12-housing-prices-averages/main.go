...
Istanbul,500,10,5,1000000`
	...
	sizesAvg := average(sizes)
	bedsAvg := average(beds)
	bathsAvg := average(baths)
	pricesAvg := average(prices)
	...
	fmt.Printf("\n%s\n", strings.Repeat("=", 75))
	fmt.Printf("%-15s", "")
	fmt.Printf("%-15.2f", sizesAvg)
	fmt.Printf("%-15.2f", bedsAvg)
	fmt.Printf("%-15.2f", bathsAvg)
	fmt.Printf("%-15.2f", pricesAvg)
	fmt.Println()
}
